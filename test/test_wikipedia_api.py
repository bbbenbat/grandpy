...
def test_wikiapi_resum_result(monkeypatch):
    """ Test the method 'resum_result' with the patching of 'mock_wikiapi_loc_result'. """
    monkeypatch.setattr('app.models.wikipedia_api.WikiApi.loc_result', mock_wikiapi_loc_result)
    req = wiki_api.resum_result()
    result = ('Tour Eiffel',
              'La tour Eiffel  est une tour de fer puddlé de 324 mètres de '
              'hauteur (avec antennes) située à Paris, à l’extrémité nord-ouest '
